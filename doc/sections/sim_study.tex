...
%Our stochastic optimization methods are less prone to getting stuck in local minima compared to the baseline methods, especially when $N=6$. The stochastic optimization methods outperform the baseline methods for all experiments with the exception of $N = 3$ and $d = 3$, where BFGS achieves the highest log-likelihood after approximately 120 epochs. However, BFGS is expected to outperform our first-order methods on a per-epoch basis when very close to the true solution. 
