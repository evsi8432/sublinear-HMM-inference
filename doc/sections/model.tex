...
To this end, we define the mappings $\widetilde \bfalpha_t$, $\widetilde \bfbeta_t$, $\widetilde \bfgamma_t$, and $\widetilde \bfxi_t$ for $t = 1,\ldots,T$ similarly to Equations (\ref{eqn:alpha}--\ref{eqn:xi}):
...
Theorem 1 below guarantees such convergence under standard regularity conditions. We prove Theorem 1 in the online appendix. %Unfortunately there is no guarantee that this stationary point will be a global (or even local) minimum, but this issue is well known in the EM literature \citep{Wu:1983}.
